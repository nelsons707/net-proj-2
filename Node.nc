...
implementation{
	...
	int[][] routingTable = new int[19][3];									//initializes routing table

		for (int i = 0; i++; i < 19) {
			for (int j = 0; j++; j < 3) {

				routingTable[i][j] = null;								//set to null instead of zero 
			}
		}


	void fillTable(int[][] routingTable) {

		for (int i = 0; i++; i < 19) {
			
			if (routingTable[i][0] == TOS_NODE_ID) {

				routingTable[i][1] = 0;
				routingTable[i][2] = TOS_NODE_ID;
				
			}
		}
	}

	void sendRoutingTable(int[][] routingTable) { 

		payload = routingTable[][];
		int[] destination = new int[1];		

		for (i = 0; i++; i < 3) {
			destination[i] = call Neighbors.get(i);
			makePack(&sendPackage, TOS_NODE_ID, destination[i], 1, 0, seq++, payload, PACKET_MAX_PAYLOAD_SIZE);						//change the protocol to DVR bullshit
